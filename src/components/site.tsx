...
import { getActiveCodes, getCodeCopyLabels } from "@/config/active-codes";
...
export async function WikiSidebar({ locale, navGroups, currentPath }: { locale: string; navGroups: NavGroup[]; currentPath?: string }) {
  ...
  const activeCodes = getActiveCodes(locale).slice(0, 5);
  const copyLabels = getCodeCopyLabels(locale);
      ...
      <section className="rounded-2xl border border-border bg-card/60 p-5">
        ...
        <div className="space-y-3 text-sm">
          {activeCodes.map((code) => (
            <div key={code.label} className="rounded-xl bg-muted p-3">
              <div className="flex min-w-0 items-start justify-between gap-2">
                <CopyCodeButton code={code.label} copyLabel={copyLabels.copy} copiedLabel={copyLabels.copied} className="px-2 py-1 text-xs" />
                <Badge className="shrink-0 bg-emerald-600 text-[10px] text-white">{code.badge}</Badge>
              </div>
              <p className="mt-1 text-muted-foreground">{code.detail}</p>
            </div>
          ))}
