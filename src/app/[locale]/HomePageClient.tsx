...
export default function HomePageClient({ home, locale, articles, recentArticles }: { home: Home; locale: string; articles: ContentItem[]; recentArticles: ContentItem[] }) {
  const activeCodes = getActiveCodes(locale);
            ...
            {home.explore.modules.map((mod) => (
              <div key={mod.order} className="overflow-hidden rounded-2xl border border-border bg-card/70">
                ...
                <div className="px-6 py-5">
                  ...
                  {mod.displayType === "code-cards" && activeCodes.length > 0 && (
                    <div className="grid gap-3 sm:grid-cols-2">
                      {activeCodes.map((h) => (
                        <div key={h.label} className="min-w-0 rounded-xl border border-border bg-muted p-4">
                          <div className="flex min-w-0 items-start justify-between gap-3">
                            <CopyCodeButton code={h.label} copyLabel={copyLabels.copy} copiedLabel={copyLabels.copied} />
                            <Badge className="shrink-0 bg-emerald-600 text-white text-[10px]">{h.badge}</Badge>
                          </div>
                          <p className="mt-3 text-sm leading-6 text-muted-foreground">{h.detail}</p>
                        </div>
                      ))}
                    </div>
                  )}
